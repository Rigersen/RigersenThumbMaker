...
def adicionar_texto_na_imagem(imagem_caminho, texto, numero, fonte_caminho, tamanho_fonte_texto, tamanho_fonte_numero, cor_texto, cor_sombra, salvar_como):
    ...
    posicao_texto = ((largura_imagem - largura_texto) / 2, (altura_imagem-100 - (altura_texto + altura_numero)) / 2)
    ...
    sombra = sombra.filter(ImageFilter.GaussianBlur(radius=6))
...
# configurações
imagem_caminho = os.path.join(diretorio_atual, 'thumb.jpg')  # diretorio da imagem
fonte_caminho = os.path.join(diretorio_atual, 'comic_sans.ttf')  # diretorio da fonte (.ttf)
tamanho_fonte_texto = 144  # Tamanho da fonte do titulo
tamanho_fonte_numero = 144  # tamanho do número
cor_texto = (255, 255, 255)  # cor do texto (RGB)
cor_sombra = (0, 0, 0, 255)  # cor da sombra (RGBA)
